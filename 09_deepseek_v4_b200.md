...
> **@1 TTFT 1.54s 偏高的原因**：① **冷启动**：@1 档位只跑 10 个正式请求，首请求命中未捕获的 batch=1 cudagraph 路径（P99=3.17s 单点拖均值）；② **EAGLE n=3 小 batch 开销**：4 token verify 在并发=1 时 amortize 不开；③ 同模型 #201（无 spec）@1 TTFT 仅 0.78s 反向佐证 EAGLE 是主因。@2 起 TTFT 自动回落到 0.83-1.06s 正常区间。
