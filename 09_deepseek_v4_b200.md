...
> ★ V4-Pro #199 / #200 的 @1 数据已用 #209 / #210 充分 warmup（n=100）后的真实值替换；原压测因 warmup n=4 不足导致虚高（1.54 / 1.51 s）。详见 §4.6 @1 TTFT 异常分析。
...
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 0.35 | 2.81 s | 1.544 s | 3.171 s | 6 ms | 7 ms | 71 | ⚠️ warmup 不足，真实 0.49s/110 toks/s（见 #209）|
...
| 阶段 | Avg TTFT | P50 | P90 | P95 | P99 | out_toks/s | 备注 |
|---|---:|---:|---:|---:|---:|---:|---|
| #199 formal (warmup n=4) | 1.544 s | — | — | — | 3.171 s | 71 | ⚠️ 失真 |
| **#209 formal (warmup n=100)** | **0.491 s** | **0.448 s** | **0.500 s** | **0.653 s** | **3.815 s** | **110** | 真实值 |

→ **真实 V4-Pro 低延迟 @1 TTFT ≈ 0.49 s，输出吞吐 110 toks/s**；#199 表中 1.54 s / 71 toks/s 是 evalscope 变长输入 × spec decoding × JIT 形状特异性的短 warmup 假象。详细推理过程、为什么 warmup n=4 不够 n=100 才行，见 **§4.6**。
...
| 并发 | RPS | Avg Lat | TTFT avg | TTFT P99 | TPOT avg | TPOT P99 | toks/s | 备注 |
|---:|---:|---:|---:|---:|---:|---:|---:|---|
| 1 | 0.26 | 3.90 s | 1.510 s | 4.082 s | 12 ms | 13 ms | 51 | ⚠️ warmup 不足，真实 0.85s/60 toks/s（见 #210）|
...
> **@1 数据失真同 §4.6**：1.51 s 是 warmup n=4 假象，DP=8 + DeepEP + EAGLE n=1 也命中变长输入 × JIT 形状特异性冷路径。详见 #210 对照实验。

#### #210 — 同 #200 配方 + warmup n=100（@1 验证）

| 阶段 | Avg TTFT | P50 | P90 | P95 | P99 | out_toks/s | 备注 |
|---|---:|---:|---:|---:|---:|---:|---|
| #200 formal (warmup n=4) | 1.510 s | — | — | — | 4.082 s | 51 | ⚠️ 失真 |
| **#210 formal (warmup n=100)** | **0.854 s** | **0.866 s** | **1.081 s** | **1.143 s** | **2.682 s** | **60.5** | 真实值 |

→ #200 配方真实 @1 TTFT ≈ **0.85 s**（原表 1.51 s 高估 77%）；输出吞吐相应回升 +19%。比 #209 V4-Pro 低延迟 (0.49s) 高约 75%，差异归因于 DP=8 + DeepEP 在 batch=1 时的 all-to-all 通信开销。@1 异常根因详见 §4.6。
...
#### @1 三种 spec 实现公平对比（修正版）

| 引擎/配置 | spec | @1 真实 toks/s | @1 真实 TTFT |
|---|---|---:|---:|
| SGLang #201 高吞吐 | 无 | 47 | 0.78 s |
| vLLM #202 baseline | MTP n=2 | 66 | 0.49 s |
| **SGLang #209 低延迟** | **EAGLE n=3**（warmup n=100 实测）| **110** | **0.49 s** |

→ 单请求场景：**SGLang + EAGLE n=3 是真正最优**（110 toks/s，是 vLLM MTP n=2 的 1.67×、SGLang 无 spec 的 2.34×）。原 §4.4 用 #201 无 spec 配置和 vLLM 比是不公平的。

**结论**：单请求 vLLM MTP 比 SGLang 无 spec 优；但 SGLang 低延迟配方（EAGLE n=3）才是 @1 真正最优。并发 ≥ 20 SGLang 高吞吐配置全面反超 vLLM。
...
### 4.6 @1 并发 TTFT 异常分析 + #209/#210 对照实验
...
#### 4.6.2 #209 / #210 对照实验（warmup n=100, formal n=100）
...
**结论性发现**：充分 warmup 后 V4-Pro + EAGLE n=3 在 @1 的真实 TTFT = **0.49s**（远低于 #199 原表 1.54s，与 #202 vLLM MTP 0.49s 完全一致；#201 无 spec 0.78s 因纯 prefill 无 spec 加速所以略高）。原 #199 @1=1.54s 是 warmup 不足的测量假象。

**#210 同方法对 #200 配方（V4-Pro 平衡 DP=8 + DeepEP + EAGLE n=1）做了交叉验证**：

| 阶段 | Avg TTFT | P50 | P90 | P95 | P99 |
|---|---:|---:|---:|---:|---:|
| #200 formal (warmup n=4) | 1.510 s | — | — | — | 4.082 s |
| #210 warmup phase n=100（自跑） | 1.512 s | 0.903 s | 2.715 s | 6.759 s | 13.502 s |
| **#210 formal (warmup n=100)** | **0.854 s** | **0.866 s** | **1.081 s** | **1.143 s** | **2.682 s** |

→ 同样 -44% 收敛。#200 配方真实 @1 TTFT = **0.85s**（高于 #209 的 0.49s 是因 DP=8 + DeepEP 在 batch=1 的 all-to-all 通信开销）。
...
- ✅ **生产真实 TTFT**：V4-Pro 低延迟 @1 真实 ≈ 0.5s（顶部 summary 与 §4.4 已用此值），#199 原始测得 1.54s 是 warmup 不足假象
- ⚠️ **压测方法学**：变长输入 + spec decoding 配置必须 `--warmup-number ≥ 100`，否则 @1 / @2 等小并发档位的 avg/P99 都失真
- ⚠️ **生产首请求 TTFT 仍略高**：服务持续运行后绝大部分路径已热，但仍有 ~1% 极端长输入（>9000 tokens）可能命中未编译的 GEMM 形状；99% 请求 TTFT 在 0.4-0.65s 区间（参考 #209/#210 P99=3.81s/2.68s）
...
| 场景 | draft_tokens | Pro 推荐 | Flash 推荐 |
|---|---|---|---|
| 低延迟 | 4（EAGLE n=3 → 节点数=1+3×1=4） | TP=8 无 DeepEP（不适用公式） | TP=8 无 DeepEP |
| 平衡 | 2（EAGLE n=1 → 节点数=1+1×1=2） | max_req=128, DISPATCH=256（128×2=256 ✓）| max_req=512, DISPATCH=1024（512×2=1024 ✓）|
| 高吞吐 | 1（无 spec → 仅 base token） | max_req=256, DISPATCH=256（256×1=256 ✓）| max_req=1024, DISPATCH=1024（1024×1=1024 ✓）|

> `draft_tokens` 即 SGLang `--speculative-num-draft-tokens` 参数，含义是验证树的总节点数（chain 拓扑 topk=1 时 = 1 + num_steps）。最大可加速 = draft_tokens（全接受时 1 个 target forward 出 N 个 token）。
...
- V4-Flash 同配置 cap 提升到 **180**（KV/slice 大 12.8× → 调度容量上限 9×）
...
| #210 | V4-Pro | SGLang | 平衡（warmup n=100 验证）| `20260427_063001_210_deepseek_v4_pro_sglang_balanced_warmup100.md` |

> #203 (V4-Flash 低延迟 旧版) 已被 #206 替代（旧版未跑到 200 档）；#204 (V4-Flash 平衡 旧版 TP=4 DP=2) 因 7.2 bug 失败未产生数据。
> #209 / #210 是 §4.6 @1 TTFT 异常的对照实验，分别验证 V4-Pro 低延迟 / 平衡配方在充分 warmup 后真实 @1 TTFT 收敛到 0.49s / 0.85s。

---

## 10. 下一步工作

- [ ] V4-Flash × vLLM 三套对照（按 §3.3 设计跑 #211-#213）
