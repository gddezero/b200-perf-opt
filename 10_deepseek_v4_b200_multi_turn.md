...
### 2.1 60K base 11 档（#211-218 + #227-229）
...
### 2.2 100K base 11 档（#219-226 + #230-232）
...
⚠️ `bytes_per_full_token=38KB` 不等于真实 KV/token，含固定开销。深度解读见 §10。

## 4. 部署：低延迟 TP=8（多轮压测唯一配方）
...
详见 [reference_v4pro_lowlat_60k_multi_turn_sla.md](../.claude/projects/-home-admin-maxwellx-altostrat-com-code-b200-vllm-opt/memory/reference_v4pro_lowlat_60k_multi_turn_sla.md)。物理深度解读见 §11。

## 5. 数据集生成（custom_multi_turn 真材料）
...
⚠️ **不要用 evalscope `random_multi_turn`** — 它每轮 append 全量 user+assistant 历史，5 turn 累计可达 1M token，与 base 概念完全不同。详见 §12。

## 6. batch_multi_turn.sh v3（含 evict 监控）
...
## 7. results_multiturn.csv（single source of truth）
...
## 10. V4-Pro 每 token KV 占用（两个口径必须区分）
...
### 10.2 口径 B：引擎物理预算（启动日志 `bytes_per_full_token`）
...
→ KV pool 占用 ≥ 90% 触发 RadixTree evict 跟不上 → 全塌方。这就是 §13 物理崩盘点公式的来源。

## 11. TP=8 dp=1 的 8 卡 KV 分布（最容易出错的地方）
...
### 11.2 1 个 logical token 在 8 卡里实际存了什么
...
→ 这就是 §4.2 表里"低延迟 TP=8 是多轮场景首选"的物理依据。

## 12. ⚠️ evalscope multi-turn 累计机制（为什么必须用 custom_multi_turn）

evalscope `random_multi_turn` 的累计行为是**最大的反直觉点**，必须先理解才能设计有效的多轮压测。

### 12.1 源码事实

**文件**：`evalscope/perf/multi_turn_benchmark.py:130-215`
...
`turn_N_input_tokens ≈ N × U`（U 是 `[min, max]` 内的采样值，max_tokens 输出 ≪ U 可忽略）
...
### 12.4 实测验证（5K-10K 单 user msg × 5 turn × 5 conv，2026-04-28）
...
### 12.5 custom_multi_turn vs random_multi_turn 对比
...
### 12.6 本压测累计长度（custom_multi_turn 真材料）
...
### 15.1 崩盘条件 = KV pool 占用 ≥ 90%（不是 100%）
...
### 17.1 ⚠️ 崩盘后 Failed 仍 = 0，监控必须看 cache hit
...
### 17.2 evalscope cache hit 估算偏乐观 14pp
...
### 17.3 percentile 必须用 P95 不用 P99
...
### 17.4 max-prompt-length 是 INCLUSION 不是 truncation
...
### 17.5 ShareGPT 短样本误导 cache hit 高估
...
### 17.6 档间不 flush_cache → 后档拿前档 cache hit
...
### 17.7 evict_per_prompt 跳变没有中间过渡
...
- evalscope 累计机制 memory：`reference_evalscope_multiturn_accumulation.md`
