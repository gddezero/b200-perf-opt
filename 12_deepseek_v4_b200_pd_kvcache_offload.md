...
### 2.4 TTFT 首 token 延迟（冷，ms，单元格 = avg / P90 / P95 / P99）

| 长度＼并发 | p8 | p16 | p32 | p64 |
|---|---|---|---|---|
| 60K  | 2574/3592/4142/11315 | 3013/4035/4467/12641 | 3683/5509/6167/7160 | 5935/8692/9779/15963 |
| 100K | 8193/14112/23854/48733 | 4144/5963/10088/16572 | 4820/6484/7394/24816 | 7622/11164/14408/33200 |
| 200K | 5343/5829/6922/43334 | 6626/8232/9435/47849 | 8751/11926/15676/47745 | **24687/43202/75390/146668** 🔴 |
| 400K | 10421/11239/30038/98916 | 12152/14089/16214/104420 | 🔴 PARTIAL（无 client summary）| (p24) 12271/17708/18819/21369 |

→ **安全档 TTFT avg 2.5–12s**；P99 长尾主要来自每 conv **首轮全量 prefill**（未命中那一段）—— 长上下文低并发尤甚（200K p8 avg 5.3s 但 P99 43.3s）。thrashing 档 200K×64：TTFT avg **24.7s / P99 146.7s**（decode 排队 + 全程 RDMA load 把 TTFT 也拖垮）。

**1st-Turn vs Subseq TTFT（avg ms，冷 / 热）—— cache/offload 价值的直接量化：**

| 档 | 1st-Turn 冷 | 1st-Turn 热 | Subseq 冷 | Subseq 热 |
|---|---|---|---|---|
| 60K p8 | 1833 | 1738 | 2600 | 2539 |
| 60K p16 | 2320 | 2385 | 3037 | 3096 |
| 60K p32 | 3059 | 3250 | 3705 | 3546 |
| 60K p64 | 6912 | 6411 | 5901 | 5701 |
| 100K p8 | **42298** | **2468** | 7017 | 3468 |
| 100K p16 | 13464 | 3789 | 3823 | 3668 |
| 100K p32 | 11848 | 4738 | 4578 | 4337 |
| 100K p64 | 16890 | 22417 | 7302 | 7419 |
| 200K p8 | **34571** | **4752** | 4335 | 4431 |
| 200K p16 | 24144 | 7334 | 6022 | 5084 |
| 200K p32 | 26429 | 10101 | 8141 | 7450 |
| 200K p64 🔴 | 45191 | 16720 | 23932 | 21097 |
| 400K p8 | **69215** | **9578** | 8394 | 7830 |
| 400K p16 | 49392 | 13569 | 10868 | 9974 |
| 400K p24 | 15658 | 16742 | 12154 | 11827 |

→ ⭐ **1st-Turn 体现「跨遍 Store 命中价值」**：冷遍每 conv 首轮要全量 prefill（100K p8 = **42.3s**、400K p8 = **69.2s**），热遍同 conv 再来时 Store/cache 命中，首轮 TTFT 骤降（100K p8 42.3s→**2.5s（17×）**、400K p8 69.2s→9.6s（7×））—— Mooncake Store 跨遍复用的最强证据。
→ **Subseq（后续轮）冷热差异小**（遍内 GPU-L1 已命中增量 prefix），冷遍稳定 4–12s。即 **1st-Turn = 跨遍 Store，Subseq = 遍内 GPU-L1**，两层 cache 各司其职。
→ 例外 100K p64：1st-Turn 热反而 ≥ 冷（22417 ≥ 16890）—— 高并发下热遍排队加剧，cache 收益被 decode 拥塞淹没（呼应 §12 冷热反转）。

### 2.5 TPOT / ITL 出 token 间隔（冷，ms，单元格 = avg / P90 / P95 / P99）

**TPOT（time per output token）：**

| 长度＼并发 | p8 | p16 | p32 | p64 |
|---|---|---|---|---|
| 60K  | 27.2/29.7/30.5/45.8 | 35.4/41.0/43.5/76.3 | 48.3/60.3/63.8/75.7 | 67.9/90.2/95.9/116.2 |
| 100K | 27.3/32.6/36.5/46.3 | 35.9/41.8/43.7/59.7 | 53.9/75.0/81.4/96.4 | 87.9/154.2/184.9/259.5 |
| 200K | 32.1/38.1/41.9/89.1 | 42.5/47.2/50.2/180.3 | 63.5/98.3/116.7/173.4 | **347.8/546.4/563.8/606.8** 🔴 |
| 400K | 34.2/40.0/43.9/141.6 | 57.3/63.5/76.1/666.6 | 🔴 PARTIAL | (p24) 58.0/84.6/104.3/151.7 |

**ITL（inter-token latency，含多轮增量 prefill 插队尖峰）：**

| 长度＼并发 | p8 | p16 | p32 | p64 |
|---|---|---|---|---|
| 60K  | 27.4/24.1/26.1/190.4 | 34.9/28.1/78.6/335.7 | 47.4/51.1/175.0/405.8 | 65.7/117.3/313.4/492.6 |
| 100K | 26.3/23.6/26.2/217.0 | 35.5/28.7/52.3/356.8 | 51.0/82.0/251.3/456.2 | 87.4/327.8/411.4/579.0 |
| 200K | 30.8/24.4/28.6/454.5 | 39.9/27.9/43.1/586.2 | 57.7/44.8/127.0/736.8 | **369.6/624.9/692.4/1023.1** 🔴 |
| 400K | 32.8/24.1/25.9/176.9 | 49.3/28.0/53.3/1021.1 | 🔴 PARTIAL | (p24) 56.5/32.5/59.7/1070.9 |

→ **TPOT 安全档 27–88ms，随并发单调升**（decode batch 变大）；⭐ thrashing 档 200K×64 = **347.8ms（健康同长度 p32 的 5.5×、60K p8 的 13×）**—— decode 每 step 等 Store RDMA load 历史 KV，出字被拖死（量化 §8）。
→ **ITL avg < P90 的反常**（如 60K p8 avg 27.4 但 P90 24.1）：稳态出字间隔很稳（二十几 ms），但每轮切换处插入**增量 prefill** 形成尖峰，把 P99 拉到 190–1071ms、avg 被尾部抬高 —— multi-turn 流式卡顿的真实位置，单看 avg 会低估。

### 2.6 RPS / 端到端 Latency（冷）

**RPS（req/s，冷 / 热）：**

| 长度＼并发 | p8 | p16 | p32 | p64 |
|---|---|---|---|---|
| 60K  | 0.506/0.523 | 0.824/0.811 | 1.246/1.277 | 1.766/1.816 |
| 100K | 0.406/0.487 | 0.757/0.783 | 1.174/1.165 | 1.364/1.523 |
| 200K | 0.409/0.423 | 0.615/0.662 | 0.955/0.989 | **0.289/0.254** 🔴 |
| 400K | 0.300/0.334 | 0.455/0.514 | 🔴 PARTIAL | (p24) 0.633/0.569 |

**端到端 per-request Latency（冷，s，单元格 = avg / P90 / P95 / P99；per-request = 单轮）：**

| 长度＼并发 | p8 | p16 | p32 | p64 |
|---|---|---|---|---|
| 60K  | 13.2/16.9/17.4/25.0 | 14.4/21.9/22.6/30.8 | 18.9/29.7/32.2/35.1 | 27.0/43.9/48.8/52.7 |
| 100K | 14.9/25.5/35.2/49.6 | 17.0/22.5/23.9/29.6 | 19.0/31.2/35.6/41.8 | 36.7/65.3/79.7/101.8 |
| 200K | 15.2/21.0/22.7/45.8 | 21.7/27.8/29.4/62.1 | 23.9/44.4/48.6/60.0 | **150.8/273.9/294.5/395.3** 🔴 |
| 400K | 22.9/26.8/45.3/101.6 | 29.0/39.0/41.5/120.0 | 🔴 PARTIAL | (p24) 29.9/46.9/49.2/52.7 |

→ **RPS 安全档随并发近线性升**（60K 0.51→1.77，p8→p64 **+3.5×**；100K 0.41→1.36 +3.4×）；⭐ thrashing 档 RPS **不升反崩**（200K p64=0.289 < p32 的 0.955，−70%）—— 并发翻倍吞吐反降是容量墙的吞吐侧表现。
→ 单请求（单轮）Latency 安全档 13–37s；thrashing 200K×64 avg **150.8s**（健康同长度的 6×）。整 conv（30 轮）per-trace 总时长安全档 ~440s（100K p8）、thrashing ~4250s（200K p64），与 wall 一致。

### 2.7 Mooncake Store I/O（冷，offload 实际流量）

**prefill save（写入 CPU pool，GB / 秒）：**

| 长度＼并发 | p8 | p16 | p32 | p64 |
|---|---|---|---|---|
| 60K  | 186/43 | 413/91 | 959/195 | 2661/466 |
| 100K | 250/45 | 474/89 | 1181/208 | 3263/528 |
| 200K | 346/86 | 682/180 | 1703/402 | 4365/766 |
| 400K | 479/154 | 1077/323 | (p32) 2308/468 🔴 | (p24) 1457/471 |

**decode load（从 CPU pool 读，GB / 秒）：**

| 长度＼并发 | p8 | p16 | p32 | p64 |
|---|---|---|---|---|
| 60K  | 18/29 | 35/60 | 71/123 | 171/261 |
| 100K | 20/28 | 38/58 | 77/127 | 315/402 |
| 200K | 24/59 | 46/125 | 110/255 | **842/2482** 🔴 |
| 400K | 34/101 | 73/213 | (p32) **410/2546** 🔴 | (p24) 106/343 |

→ **save ≫ load**：prefill 把每轮 KV 写入 Store（save），但 decode 当轮 KV 走 **P→D RDMA 直传**，只有「跨轮复用历史 KV」才走 Store load —— 安全档 load 仅 save 的 ~6–10%（thrashing 档因 decode 全程 load 飙到 19%，见 200K p64）。
→ ⭐ **thrashing 铁证（量化 §8 容量墙）**：decode load_time 200K×64 = **2482s**、400K×32 = **2546s**，占各自 wall（6273s / 6093s）的 **40% / 42%**，是健康档（几十–几百 s）的 ~10×；load bytes 同步暴涨到 842 / 410 GB（健康档 ~100GB）。decode-L1 崩 → 每 step 从 Store RDMA load → load_time 吃满 wall。
→ Store 占用峰值 ~2.0 TB（< 3.43 TB 总容量，未满 → 排除「容量耗尽」，见 §8.3）。
...
6. **TTFT 尾延迟由「首轮全量 prefill」主导**：安全档 TTFT avg 2.5–12s 但 P99 可达 40–100s（长上下文低并发），全是每 conv 首轮 cold prefill；热遍 Store 命中后首轮 TTFT 降 7–17×（§2.4）。
7. **TPOT 是 thrashing 最灵敏的延迟判据**：安全档 27–88ms 与并发弱相关，撞墙档暴涨到 348ms（5–13×），比 wall（6×）更早、更线性地反映 decode 被 Store RDMA 拖死。
8. **ITL P99 尖峰 = 多轮增量 prefill 插队**：稳态出字 ~25ms 但每轮切换处 P99 冲到 190–1071ms，avg 被尾部抬高（avg > P90）；流式卡顿在轮切换处，不在稳态。
...
**prefill-bound 延迟 / 吞吐绝对值（冷 / 热）—— offload 价值的延迟侧证据：**

| 并发 | RPS 冷/热 | TTFT 冷/热(ms) | TPOT 冷(ms) | per-req Lat 冷/热(s) |
|---|---|---|---|---|
| p8  | 1.224/1.800 | 3918/2528 | 30.7 | 5.4/4.0 |
| p16 | 2.201/2.720 | 4123/3528 | 38.7 | 5.9/5.5 |
| p32 | 3.084/3.782 | 5922/5344 | 70.5 | 9.2/8.0 |
| p64 | 2.471/4.712 | 16159/8522 | 54.0 | 18.6/12.4 |

→ ⭐ **热遍 RPS 大幅高于冷遍**（p64 **2.471→4.712，+91%**），TTFT 同步 **−47%（16159→8522ms）**—— Store 跨遍命中直接把吞吐近翻倍，量化 §10「offload 是 prefill-bound 主引擎」。对比 decode-bound 主 sweep（§2.6）热遍 RPS 变化多在 ±10% 内，差异即「offload 价值 = workload」的延迟侧铁证。
→ prefill-bound RPS 绝对值远高于同长度 decode-bound（100K p64 prefill-bound 4.712 vs decode-bound 1.523，3.1×），因输出仅 ~46 tok（§方法论 caveat），decode 几乎不占时间。
...
>
> **延迟指标补充（2026-06-01）**：§2.4–2.7 新增 TTFT / TPOT / ITL / Latency（各 avg·P90·P95·P99）+ 1st/Subseq TTFT 冷热 + RPS + Mooncake Store I/O（save/load bytes·time）矩阵，§10 新增 prefill-bound 延迟/RPS 绝对值表。全部数字经**确定性脚本逐单元格核实**（延迟/RPS/1st-Subseq 87 + Store I/O 32 + prefill-bound 表抽查，对齐 `outputs/**/benchmark_summary.json` + `benchmark_percentile.json` + `metrics/prom_*_summary.txt`，**100% 吻合**）；自查订正 2 处解读区间（Store load 占比 ~6–10%、decode-bound 热遍 RPS ±10%）。真值源同上。
